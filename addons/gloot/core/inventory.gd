...
@export var grid_constraint: GridConstraint
...
func get_property_from_prototree(item_id: String, property_to_get : String):
	return _prototree.get_prototype_property(item_id,property_to_get,null)
...
## cannot be added.
func create_and_add_item(prototype_id: String, amount : int = 1) -> InventoryItem:
	var item: InventoryItem = InventoryItem.new(protoset, prototype_id)
...
func create_and_add_item2(prototype_id: String, amount : int = 1) -> InventoryItem:
	var item: InventoryItem = InventoryItem.new(protoset, prototype_id)
