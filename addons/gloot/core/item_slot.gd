...
func _on_item_added(item: InventoryItem) -> void:
	print("Emitting from:", self)
	print("Emitting signal:", item_equipped)
	item_equipped.emit()
...
func equip(item: InventoryItem) -> bool:
	...
	print_debug("Zalozone")
	return true
	
	
func is_type_correct(item: InventoryItem) -> bool:
	if SlotType.keys()[slot_type].to_lower() == "none":
		return true
	if item.get_type().to_lower() == SlotType.keys()[slot_type].to_lower():
		return true
	return false
...
func clear_and_return() -> bool:
	if get_item() == null:
		return false
	
	get_parent().spawn_new_item(get_item())
